...
# Install packages that are broken in pixi
...
create_tsne_plot <- function(data_beta, data_meta, perplexity = 4, max_iter = 100) {
  # Get test_id for color scheme
...
#' @param data_beta Beta values
#' @param data_meta Metadata
#' @return ggplot object of PaCMAP plot
create_pacmap_plot <- function(data_beta, data_meta) {
  # Get test_id for color scheme
  test_id <- setdiff(unique(data_meta$Status), 
                     c("in_silico_case", "control", "proband"))
  ...
  # Set pixi python and import PaCMAP
  ...
  pacmap_df <- data.frame(
    IDs = colnames(data_beta),
    pacmap1 = embedding[,1],
    pacmap2 = embedding[,2]
  )
  
  # Merge with metadata
  pacmap_df <- merge(pacmap_df,
                     data_meta,
                     by = "IDs")
  
  # Create the plot
  ggplot(pacmap_df) +
    geom_point(pacmap_df[pacmap_df$Status != "proband",], 
               mapping=aes(pacmap1, pacmap2,
                           color = Status,
                           shape = Status == "proband", 
                           size = 1,
                           alpha = 0.85)) +
